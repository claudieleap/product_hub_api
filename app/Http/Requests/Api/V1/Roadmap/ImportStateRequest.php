...
class ImportStateRequest extends RoadmapRequest
{
    public function rules(): array
    {
        return [
            'items' => 'nullable|array',
            'items.*.id' => 'required_with:items|string|max:120',
            'items.*.productId' => 'required_with:items|string|max:80',
            'items.*.priority' => 'required_with:items|in:alta,media,baixa,perfumaria',
            'items.*.title' => 'required_with:items|string|max:500',
